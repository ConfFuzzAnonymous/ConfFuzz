...
#define RUN_PATH "./run_ob.sh"
#define PREPARE_PATH "./prepare.sh"
#define SHM_PATH "./shm.info"
#define STOP_PATH "./stop.sh"
...
// wangbo
#define MAX_SQL_NUM 30
IRTYPE cur_ir_type = kProgram;
string valid_sqls = "";
int valid_sql_nums = 0;
...
public:
  ...
  int connect()
  {
    ...
    if (mysql_real_connect(&m_, host_, user_name_, passwd_, dbname.c_str(), 0, NULL, CLIENT_MULTI_STATEMENTS) == NULL)
    {
      ...
      // // writeLog("afl_fuzz.sql", mysql_error(&m_));
      // // writeLog("afl_fuzz.sql", to_string(error_num).c_str());
  ...
  SQLSTATUS execute(char *cmd)
  {
    ...
    // // writeLog("afl_fuzz.sql", log_interesting.c_str());
    ...
    SQLSTATUS sql_status = kNormal;

    if (error_code == 1064)
    {
      sql_status = kSyntaxError;
    }
    else if (error_code != 0)
    {
      sql_status = kSemanticError;
    }
    else
    {
      sql_status = kNormal;
    ...
    if (check_res == false)
    {
      // writeLog("afl_fuzz.sql", "-- check_server_alive works");
      return kServerCrash;
    }
  ...
  {
    auto conn = connect();
    // // writeLog("afl_fuzz.sql", "connect:");
    // // writeLog("afl_fuzz.sql", to_string(conn).c_str());
  ...
  void init_database()
  {
    ...
    string clear_query = "DROP DATABASE IF EXISTS test1 ;\n";
    clear_query += "CREATE DATABASE IF NOT EXISTS test1 ;\n";
    clear_query += "USE test1 ;\n";
    // writeLog("afl_fuzz.sql", clear_query.c_str());
    auto tmp_res = mysql_real_query(&m_, clear_query.c_str(), clear_query.size());
    int res = -1;
    do
    {
    ...
    // writeLog("afl_fuzz.sql", reset_query.c_str());
    tmp_res = mysql_real_query(&m_, reset_query.c_str(), reset_query.size());
    res = -1;
    do
    {
      auto q_result = mysql_store_result(&m_);
...
{
  // static u8 origin_trace_bits[MAP_SIZE];
  static struct itimerval it;
  ...
  // memcpy(origin_trace_bits, trace_bits, MAP_SIZE);
  MEM_BARRIER();
BEGIN:
...
#endif
  // if (result == kSyntaxError || result = kSemanticError) {
  //   memcpy(trace_bits, origin_trace_bits, MAP_SIZE);
  // }
  ...
  {
    // writeLog("afl_fuzz.sql", "-- Connection failed");
    sleep(1);
    goto BEGIN;
  }
  else
                         ...
                         u32 handicap, u8 from_queue)
{
  ...
  for (stage_cur = 0; stage_cur < 1; stage_cur++)
  {
    ...
    // memcpy(trace_bits, trace_bits, MAP_SIZE);
    // writeLog("afl_fuzz.sql", ("-- calibrate exec_cksum = " + to_string(hash32(trace_bits, MAP_SIZE, HASH_CONST))).c_str());
    ...
    if (q->exec_cksum != cksum)
    {
      ...
      // writeLog("afl_fuzz.sql", ("-- calibrate has_new_bits = " + to_string(hnb)).c_str());
      if (hnb > new_bits)
        new_bits = hnb;
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault)
{
  ...
  // // writeLog("afl_fuzz.sql", "g_if_interest = 1");
  g_if_interest = 1;
  ...
  // // writeLog("afl_fuzz.sql", ("fault" + to_string(fault)).c_str());
  ...
  if (fault == crash_mode)
  {
    ...
    /* Keep only if there are new bits in the map, add to queue for
       future fuzzing, etc. */
    // writeLog("afl_fuzz.sql", ("-- first exec_cksum = " + to_string(hash32(trace_bits, MAP_SIZE, HASH_CONST))).c_str());
    if (!(hnb = has_new_bits(virgin_bits)))
    {
      // writeLog("afl_fuzz.sql", "has_new_bits = 0");
      g_if_interest = 0;
      if (crash_mode)
        total_crashes++;
      return 0;
    }
    // writeLog("afl_fuzz.sql", ("has_new_bits = " + to_string(hnb)).c_str());
#ifndef SIMPLE_FILES
    ...
    // queue_top->sql_nums = valid_sql_nums;

    /* Try to calibrate inline; this also calls update_bitmap_score() when
       successful. */
    res = calibrate_case(argv, queue_top, mem, queue_cycle - 1, 0);
    // // writeLog("afl_fuzz.sql", "g_if_interest = 0");
    g_if_interest = 0;
  ...
  if (fault == FAULT_CRASH)
  {
    ...
    reset_query += "INSERT INTO v4 VALUES (4, 5, 6) ;\n";
    reset_query += "INSERT INTO v4 VALUES (6, 5, 4) ;\n";
    ck_write(fd, reset_query.c_str(), reset_query.size(), fn);
  }
  ck_write(fd, mem, len, fn);
  close(fd);
...
#define SP20 SP10 SP10
  ...
  SAYF(bV bSTOP "     kNormal : " cRST "%-37d " bSTG bV bSTOP "    levels : " cRST "%-10s " bSTG bV "\n", g_kNormal_count, DI(max_depth));
  ...
  SAYF(bV bSTOP "kSemanticError:" cRST "%-37d " bSTG bV bSTOP "   pending : " cRST "%-10s " bSTG bV "\n", g_kSemanticError_count, DI(pending_not_fuzzed));
  ...
  SAYF(bV bSTOP "kSyntaxError : " cRST "%-37d " bSTG bV bSTOP "  pend fav : " cRST "%-10s " bSTG bV "\n", g_kSyntaxError_count, DI(pending_favored));
...
//   /* Select initial chunk len, starting wirun_targetth large steps. */
              ...
              if(s == "Engine"){
                 ...
                 size_t createTablePos = g_current_input.find("CREATE TABLE");
                  ...
                  if (semicolonPos != std::string::npos) {
                     ...
                     std::cout << "First (;) location " << semicolonPos << std::endl;
...
static u8 fuzz_one(char **argv)
{
  ...
  for (auto &ir : mutated_tree)
  {
    ...
    extract_struct(ir);
    g_current_ir = ir;
    stage_name = "not named yet";
    // writeLog("afl_fuzz.sql", "-- mutated");
    // writeLog("afl_fuzz.sql", ir->to_string().c_str());

    vector<IR *> v_kStmt; 
    IR *node_kStmtlist = ir->left_;
    ...
    {
      cur_ir_type = node_kStmt->left_->type_;

      bool tmp_res = my_validate(node_kStmt);
      if (tmp_res == false)
      {
        continue;
      }

      string ir_str = node_kStmt->to_string();

      int siz = ir_str.size();
      // writeLog("afl_fuzz.sql", "-- validate");
      // writeLog("afl_fuzz.sql", ir_str.c_str());

      if (common_fuzz_stuff(argv, ir_str.c_str(), siz))
      {
        goto abandon_entry;
      }

      if (g_cur_status == kNormal)
      {
#ifdef __AFL_DEBUG__
        writeLog("afl_fuzz.log", "\ncur_sql_type: ");
        writeLog("afl_fuzz.log", get_string_by_nodetype(cur_ir_type).c_str());
        writeLog("afl_fuzz.log", " cur sql: ");
        writeLog("afl_fuzz.log", ir_str.c_str());
        writeLog("afl_fuzz.log", "\n");
#endif
        apply_changes(cur_ir_type); 
      }
      valid_sqls += ir_str;
      valid_sqls += "\n";
      valid_sql_nums += 1;
    ...
    node_kStmtlist = ir->left_;
    for (auto &node_kStmt : v_kStmt)
    {
      node_kStmtlist->left_ = node_kStmt->left_->left_;
      node_kStmt->left_->left_ = NULL;
      deep_delete(node_kStmt);
      node_kStmtlist = node_kStmtlist->right_;
    }
    ...
    u8 fault = FAULT_NONE;
    if (g_cur_status == kServerCrash)
    {
      fault = FAULT_CRASH;
    }
    else if (g_cur_status == kTimeout)
    {
      fault = FAULT_TMOUT;
    }
    if (valid_sql_nums == 0)
    ...
    int should_keep = save_if_interesting(argv, valid_sqls.c_str(), valid_sqls.size(), fault);
