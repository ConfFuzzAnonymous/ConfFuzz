...
vector<IR *> Mutator::havoc_mutate(vector<IR *> &v_ir_collector, int energy, vector<int> &g_havoc_count)
{
    ...
    if (energy < HAVOC_MUTATE_MIN)
        energy = HAVOC_MUTATE_MIN;
...
{
    if (cur->left_)
        add_ir_to_library_no_deepcopy(cur->left_, ir_library_index);
    if (cur->right_)
        add_ir_to_library_no_deepcopy(cur->right_, ir_library_index);
