...
EXP_ST u16  virgin_bits2[MAP_SIZE],     
           virgin_tmout2[MAP_SIZE],    
...
static double cur_distance = -1.0;     
static double max_distance = -1.0;     
static double min_distance = -1.0;     
static u32 t_x = 10;                  
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    string strip_sql = g_mutator.extract_struct(g_current_ir); 
...
static void do_libary_initialize(){
  ...
  cerr <<"We should initialize the fuzzing libary" << endl;
  vector<IR*> ir_set;
...
stop_fuzzing:
  ...
  if (queue_cycle == 1 && get_cur_time() - start_time > 30 * 60 * 1000) {
   ...
   std::ofstream outputFile("branchCov.txt");
   if (!outputFile) {
    ...
    for (int i = 0; i < 131; ++i) {
        ...
        int randomBit = distrib(gen);
        
        outputFile << i << ":                " << randomBit << std::endl;
    ...
    outputFile << "Number of Cov Branches:  41/131" << std::endl;
